		...
		if($this->id > 0){ ?>
			...
			?>
				<li class="dropdown">
					...
					<div class="dropdown-menu dropdown-menu-right" aria-labelledby="alertsDropdown">
						...
						<a class="dropdown-item" href="/admin/categories/admin.html">Categorias</a>
			...
			<?php 
				...
				<li class="dropdown">
					...
					<div class="dropdown-menu dropdown-menu-right" aria-labelledby="alertsDropdown">
						<a class="dropdown-item dropdown-menu-right" href="/demo/index.html"><span>Index</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/404.html"><span>404</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/blank.html"><span>blank</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/charts.html"><span>charts</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/contact.html"><span>contact</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/education.html"><span>education</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/entertain.html"><span>entertain</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/living.html"><span>living</span></a>
						<a class="dropdown-item dropdown-menu-right" href="/demo/tables.html"><span>tables</span></a>
						<div class="dropdown-divider"></div>
...
{	
	...
	function __construct()
	{
		$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $pdo->prepare('SELECT `url_redirects`.* FROM `url_redirects` ORDER BY `url` ASC LIMIT 1000');
		$stmt->execute();
		$result = ($stmt->fetchAll(PDO::FETCH_OBJ));
		$temp =  array();
		$this->total = count($result);
...
class LogsAuth
{
	...
	function __construct($limit=1000)
	{
		$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $pdo->prepare('SELECT `log_auth`.* FROM `log_auth` LIMIT ' . $limit);
		$stmt->execute();
		$result = ($stmt->fetchAll(PDO::FETCH_OBJ));
		$temp =  array();
		$this->total = count($result);
...
function formatVideo($inputFormat)
{
	switch($inputFormat)
	{
		case 'video/mp4':
...
class VideoYT extends BaseClass
{
	var $ref, $title, $thumb, $videos;
	...
	function __construct($params)
	{
		...
		)
		{
			...
			$this->load_by_ref($params->ref);
		}
	}
	
   function load_by_ref($ref)
   {
		$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $pdo->prepare('SELECT `videos_yt`.*
		FROM `videos_yt` 
		WHERE `videos_yt`.`ref` = ?');
		#$pdo->exec("SET CHARACTER SET utf8; SET COLLATION SET utf8_unicode_ci");
		$stmt->execute([$ref]);
		...
		if(isset($result[0])){
			$resultOne = $result[0];
			$this->setData($resultOne);
			$this->load_videos_by_id($resultOne->id);
		}else{
			try
			{
				$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql = "INSERT INTO `videos_yt` (`ref`,`title`,`thumb`)
				VALUES ('{$this->ref}','".utf8_decode($this->title)."','{$this->thumb}')";
				$pdo->exec($sql);
				$this->load_by_ref($ref);
			}
			catch(PDOException $e)
			{
				# echo $sql . "<br>" . $e->getMessage();
   ...
   function load_videos_by_id($video_id)
   {
		$videoT = array();
	   foreach($this->videos as $item)
	   {
		   if(isset($item['file']))
		   {
			   $videoT[] = $this->load_video_by_ref_and_type($video_id, $item);
		   }
	   }
		$this->videos = $videoT;
   }
   
   function load_video_by_ref_and_type($video_id, $video)
   {
		$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $pdo->prepare("SELECT `videos_yt_files`.*
		FROM `videos_yt_files` 
		WHERE `videos_yt_files`.`video` = '{$video_id}' AND `videos_yt_files`.`file` = '{$video['file']}'");
		#$pdo->exec("SET CHARACTER SET utf8; SET COLLATION SET utf8_unicode_ci");
		$stmt->execute();
		$result = ($stmt->fetchAll(PDO::FETCH_OBJ));
		if(isset($result[0])){
			...
			try
			{
				...
				$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
				$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql = "INSERT INTO `videos_yt_files` (
					`video`
					,`title`
					,`poster`
					,`file`
				...
				VALUES (
					...
					,'{$video['quality']}')";
				$pdo->exec($sql);
				$this->load_video_by_ref_and_type($video_id, $video);
			}
			catch(PDOException $e){
				#echo $sql . "<br>" . $e->getMessage();
				#exit();
...
class CategoriesList extends BaseClass
{
	var $total = 0;
	var $list = array();
	...
	function __construct()
	{
		$pdo = new PDO("mysql:host=".HOST_DB.";dbname=".NAME_DB, USER_DB, PASS_DB);
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $pdo->prepare('SELECT `categories`.*, `C2`.`name` AS `root_name` FROM `categories`
			LEFT JOIN `categories` AS C2 ON `C2`.`id` = `categories`.`root`
			ORDER BY root ASC
		LIMIT 1000');
		$stmt->execute();
		$result = ($stmt->fetchAll(PDO::FETCH_OBJ));
